...
class StridingSubsampling(nn.Module):
    """
    Слой предварительного понижения размерности свёрточными сетями.
    зафисируем subsampling factor = 4, как в статье.
    линейный слой  вынесем наружу, чтобы как накартинках в статье
    """
...
class RelPositionalEmbedding(nn.Module):
    ...
    def create_pe(self, length: int, device: torch.device) -> Tensor:
        # Создать позиционные индексы для относительных позиций
        # позиции: от -(L-1) до +(L-1), всего 2L-1
        ...
        # Вычислить коэффициенты для тригонометрических функций
        ...
        # Применить синус и косинус к четным и нечетным позициям
        pe[:, 0::2] = torch.sin(angles)  # чётные индексы
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        center = full_len // 2              # индекс нуля (0-смещение)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward_attention(self, value: Tensor, scores: Tensor, mask: Optional[Tensor]):
        ...
        if mask is not None:
            ...
            #attn = torch.softmax(scores, dim=-1)  #TODO: достаточно же и так?
...
class RelPositionMultiHeadAttention(MultiHeadAttention):
    ...
    def __init__(
        ...
        # Линейное преобразование для позиционных эмбеддингов
        self.linear_pos = nn.Linear(input_dim, input_dim, bias=False)

        # Параметры позиционных смещений u и v
...
class MultiHeadedSelfAttentionModule(nn.Module):
    def __init__(
        ...
        self.pos_enc = RelPositionalEmbedding(
            dim=input_dim,
            base= 10000,              # у нас короткие записи, возьмем поменьше базу.
            init_len=max_len          # длина которая будет расчитана при инициализации
        )
...
class ConvolutionModule(nn.Module):
    ...
    плюс еще по статье тут есть LayerNorm в начале и Dropout в конце
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert (kernel_size - 1) % 2 == 0,  "kernel_size должен быть нечётным"

        self.ln = nn.LayerNorm(in_channels)

        # pointwise conv с расширением каналов в 2 раза
        ...
        # depthwise conv (groups = in_channels) с паддингом, чтобы не менять длину
        ...
        # финальный pointwise conv обратно в in_channels каналов
    ...
    def forward(self, x: Tensor, pad_mask: Optional[Tensor] = None) -> Tensor:
        ...
        x = x.transpose(1, 2)    # x: [B, T, C] -> [B, C, T]
        # Первый блок свёрточного модуля
        #       pointwise conv -> GLU
        x = self.pointwise_conv1(x)
        x = self.glu(x)

        # Маскируем паддинг индексы, заменяем их на нули
        # pad_mask: [B, T] (True там, где паддинг)
        if pad_mask is not None:
            x = x.masked_fill(pad_mask.unsqueeze(1), 0.0)

        # Второй блок свёрточного модуля
...
class ConformerBlock(nn.Module):
    ...
    1/2 x FFN -> RelPositional Multi-Head Attention -> Conv -> 1/2 x FFN
    ...
    def forward(
        ...
        """
        x:        [B, T, encoder_dim]
        pos_emb:  [B, T, encoder_dim]
        att_mask: [B, T, T] или [T, T] (опционально)
        pad_mask: [B, T] (True там, где паддинг)
        """

        # 1) Первая 1/2 FFN (Macaron)
        #    FFN внутри уже делает layer norm
        ...
        # 3) Convolution module (внутри уже есть LayerNorm и Dropout)
...
class ConformerEncoder(nn.Module):
    ...
    def forward(self, audio_signal: Tensor, lengths: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if B > 1:
            ...
            att_mask = pad_mask.unsqueeze(1) | pad_mask.unsqueeze(2)   # [B, T', T'] - True там, где паддинг
