...
class NoisyBucketBatchSampler(Sampler):
    def __init__(self, lengths, batch_size, shuffle=True, sort=True, sample_weights=None):
        ...
        sort: сортировать ли по длине (bucket batching).
              True  + shuffle=True  — noisy bucket sort + shuffle батчей
              True  + shuffle=False — детерминированная сортировка по длине
              False + shuffle=True  — полный random (все индексы перемешиваются)
              False + shuffle=False — последовательный порядок
    ...
    def __iter__(self):
        ...
        # порядок батчей перемешать (при sort=True внутри батча порядок по длине сохраняется)
